...
//        System.err.printf("Analyzing: %s\n", lst);
    ...
    protected void onMatchEnded(int currentTurn, int i, int j, Optional<Symbols> winner) {
        matchEnded().emit(new MatchData(this, currentTurn, winner.orElse(null), i, j));
    }

    protected void onTurnEnded(int currentTurn, int i, int j, Symbols currentPlayer) {
        turnEnded().emit(new TurnData(this, currentTurn, currentPlayer, i, j));
        turn++;
        player = nextPlayer(player);
        turnBeginning().emit(new TurnData(this, getTurn(), getPlayer(), i, j));
    }
